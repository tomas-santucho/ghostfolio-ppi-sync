...
test('prefers account, currency, market and ISIN-specific overrides',()=>{
  const global={symbol:'AAPLD',mappedSymbol:'AAPL',dataSource:'YAHOO' as const};
  const scoped={symbol:'AAPLD',mappedSymbol:'8b7b0cef-0b7b-4a3a-a8b1-c0e9bc1a1401',accountId:'ppi-a',currency:'ARS' as const,market:'BYMA',isin:'ARDEUT116183',dataSource:'MANUAL' as const};
...
test('rejects ambiguous selectors and invalid provider identities before importing',()=>{
  ...
  expect(()=>parseSymbolOverrides([{symbol:'AL30',mappedSymbol:'AL30',dataSource:'MANUAL'}])).toThrow('UUID or GF_');
