...
test('validates and maps configuration', () => expect(loadConfig(env)).toMatchObject({ dryRun:false, syncFromDate:new Date('2024-01-01T00:00:00.000Z'), ppi:{orderEnrichment:false} }));
test('parses opt-in manual cash assets',()=>expect(loadConfig({...env,PPI_CASH_ASSETS:'{"ARS":"PPI_CASH_ARS","USD_MEP":"PPI_CASH_USD_MEP"}'}).cashAssets).toEqual({ARS:'PPI_CASH_ARS',USD_MEP:'PPI_CASH_USD_MEP'}));
test('rejects an invalid manual cash asset map',()=>expect(()=>loadConfig({...env,PPI_CASH_ASSETS:'{"USD":"PPI_CASH_USD"}'})).toThrow());
