...
export function transactionFingerprint(transaction:NormalizedTransaction):string { const canonical=['ppi',transaction.accountId,transaction.externalId??'',transaction.type,transaction.date.toISOString(),transaction.symbol??'',transaction.quantity??'',transaction.unitPrice??'',transaction.fee??'',transaction.currency].join('|'); return createHash('sha256').update(canonical,'utf8').digest('hex'); }
