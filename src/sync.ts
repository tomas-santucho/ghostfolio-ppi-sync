...
export async function runSync(ppi:Pick<PpiClient,'getTransactions'|'getOrders'|'searchInstrument'>,ghostfolio:GhostfolioClient,options:{ppiAccountId:string;ghostfolioAccountId:string;from?:Date;dryRun:boolean;enrichOrders?:boolean;symbolOverrides?:SymbolOverride[];cashAssets?:CashAssetMap;warn?:(message:string)=>void}):Promise<SyncSummary> {
  let transactions=await ppi.getTransactions({accountId:options.ppiAccountId,from:options.from});
  if (options.enrichOrders && ppi.getOrders) transactions=enrichTransactionsWithOrderIds(transactions,await ppi.getOrders({accountId:options.ppiAccountId,from:options.from}));
